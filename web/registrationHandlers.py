...
class IntimateRegisterRequestHandler(BaseHandler):
    def get(self, ek):
        ...
        try:
            if self.user:
                if continue_url:
                    return self.redirect_to('addressRegister', uk=self.user_key.urlsafe(),  continue_url=continue_url)
                else:
                    return self.redirect_to('addressRegister', uk=self.user_key.urlsafe())
            ...
            try:
                ...
                if not emailModel:
                    ...
                    self.add_message(message, 'error')
                    if continue_url:
                        return self.redirect_to('softRegister', ck=str(urlsafeEmailKey), continue_url=continue_url)
                    else:
                        return self.redirect_to('softRegister', ck=str(urlsafeEmailKey))
            ...
            if continue_url:
                return self.redirect_to('softRegister', continue_url=continue_url)
            else:
