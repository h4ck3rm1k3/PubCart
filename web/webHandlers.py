...
class AddToWatchlistHandler(BournEEHandler):
    ...
    def post(self):
        try:
            ...
            object_key = str(self.addToWatchlist_form.ok.data.strip())
            wl_name = str(self.addToWatchlist_form.wln.data.strip()).upper()
            if wl_name is None:
                wl_name = 'ALL'
            ##: Try to find the part in the Datastore.
            objectKey = ndb.Key(urlsafe=object_key)
            objectModel = objectKey.get()

            if objectModel:
                ##: Check for existing WatchListItem Model
                watchlist = shoppingModels.Watchlist.get_or_create_watchlist(self.user_key, wl_name)
                if not watchlist:
                    raise Exception('Error, could not get or create a watchlist with function: get_or_create_watchlist')

                wl_items = watchlist.kl

                if objectKey in wl_items:
                    message = _('Item: {}, is already in your Watchlist'.format(objectKey.id()))
                    self.add_message(message, 'success')
                else:
                    wl_items.append(objectKey)
                    watchlist.kl = wl_items
                    completedKey = watchlist.put()
                    if completedKey:
                        message = _('Item: {}, has been added to your Watchlist'.format(objectKey.id()))
                        self.add_message(message, 'success')
                    else:
                        logging.error('Error, No completedKey returned from .put() in function POST of CompleteExchangeOrderHandler')
                ...
                message = _('We are having difficulties with adding this item to your Watchlist. Please try again later.')
                self.add_message(message, 'error')

        except Exception as e:
            logging.error('Error in function POST of AddToWatchlistHandler : {}'.format(e))
            message = _('We are having difficulties with adding the item to your Watchlist. Please try again later.')
            self.add_message(message, 'error')
        finally:
            try:
                self.redirect(self.request.referer)
        ...
        try:
            ...
            try:
                ...
                if watchlist:
                    ...
                    logging.error('Error in function POST of DeleteFromWatchlistHandler')
