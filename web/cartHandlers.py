        ...
        try:
            ...
            if allPublicCarts:
                for cart in allPublicCarts:
                    if cart.img is None:  # TODO remove the None
                        carts.append(cart)
...
class FullPageCartHandler(RegisterBaseHandler):
    ...
    def public_cart(self, userID, cartName):
        try:
            ...
            if cartOwnerInfo:
                ...
                cartKey = ndb.Key(shoppingModels.Cart, str(cartName).upper(), parent=cartOwnerKey)
    ...
    def do_work(self, cart):
        try:
            ...
            if cart.n:
                self.forkCart_form.name.data = cart.n
            if cart.d:
    ...
    def post(self, urlsafeCartKey):
        try:
            if not self.cartDetails_form.validate():
                ...
                message = _("The submission to edit the cart's details was not valid.")
                self.add_message(message, 'error')
            ...
            if urlsafeCartKey == formUrlsafeCartKey:
                ...
                if cart:
                    ...
                    message = _('The cart details have benn saved.')
                    self.add_message(message, 'success')
    ...
    def post(self, urlsafeCartKey):
        try:
            if not self.forkCart_form.validate():
                ...
                message = _('The submission to fork the cart was not valid.')
                self.add_message(message, 'error')
                try:
                    self.redirect(self.request.referer)
                except:
                ...
                if urlsafeCartKey == formUrlsafeCartKey:
                    ...
                    if cart:
                        ...
                        if cart.key == forkedCartKey:
                            ...
                            self.redirect_to('fullPageCart', urlsafeCartKey=forkedCartKey.urlsafe)
                    else:
                        ##: If we get here there has been an error
                        raise Exception("A Cart could not be found with the given urlsafeCartKey.")
                else:
                    ##: If we get here there has been an error
                    raise Exception("Either the cartKeys did not match or we could not find a Cart with the key.")
