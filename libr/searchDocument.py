...
from libr import exceptions as errors
...
class BaseDocumentManager(object):
	...
	_INDEX_NAME = None
	_VISIBLE_PRINTABLE_ASCII = frozenset(set(string.printable) - set(string.whitespace))

	def __init__(self, doc):
		"""
		Builds a dict of the fields mapped against the field names, for efficient access.
		"""
		self.doc = doc
		fields = doc.fields
...
class Seller(BaseDocumentManager):

	_INDEX_NAME = utils.SELLER_INDEX_NAME
...
class Product(BaseDocumentManager):
	...
	_INDEX_NAME = utils.PRODUCT_INDEX_NAME
	...
	_SORT_OPTIONS = [
		...
		[BEST_PRICE, 'best price', search.SortExpression(
			# other examples:
			...
			expression=BEST_PRICE,
			direction=search.SortExpression.ASCENDING, default_value=9999)],
		[LAST_PRICE, 'last price', search.SortExpression(
		...
		[MANUFACTURER, 'manufacturer', search.SortExpression(
	...
	def updateRatingsInfo(cls, doc_id, avg_rating):
		"""Given a shoppingModels.Product entity, update and reindex the associated
...
# 'accessor' convenience methods
	...
	def getProductNumber(self):
		...
		return self.getFieldVal(self.PRODUCT_NUMBER)
	...
	def _buildCoreProductFields( \
		...
		fields = [	search.AtomField(name=cls.PRODUCT_NUMBER, value=product_number),
					...
					search.TextField(name=cls.PRODUCT_NAME, value=product_name),
					search.TextField(name=cls.MANUFACTURER, value=manufacturer),
					...
					value=re.sub(r'<[^>]*?>', '', product_description)),
	...
	def _createDocument(
		...
		if urlsafeProductKey and category_name and product_number:
			# First, check that the given urlsafeProductKey has only visible ascii characters,
			# and does not contain whitespace.	The urlsafeProductKey will be used as the doc_id,
			# which has these requirements.
			if not cls.isValidDocId(urlsafeProductKey):
			...
			# auto-generated.)
			d = search.Document(doc_id=urlsafeProductKey, fields=resfields)
			return d
		else:
